...
class TablePrompt extends Base {
  ...
  dimen2ArrToObj(dimenArr) {
    ...
    rowNames.forEach((rowName, rowIndex) => {
      columnNames.forEach((columnName, columnIndex) => {
        obj[`${rowName}_${columnName}`] =
          (dimenArr && dimenArr[rowIndex] && dimenArr[rowIndex][columnIndex]) ||
          "";
      });
    });
    return obj;
  ...
  onLeftKey() {
    ...
    this.column = this.column > 0 ? this.column - 1 : length - 1;
  ...
  onRightKey() {
    ...
    this.column = this.column < length - 1 ? this.column + 1 : 0;
    this.render();
  }
  ...
  handleDelete() {
    if (
      this.values[this.row] &&
      this.values[this.row][this.column] &&
      this.values[this.row][this.column].length > 0
    ) {
      this.values[this.row][this.column] = this.values[this.row][
        this.column
      ].substring(0, this.values[this.row][this.column].length - 1);
    }
    this.render();
  }
  ...
  render(error) {
    ...
    const table = new Table({
      head: [
        ...
        this.columns.pluck("name").map((name, index) => {
    ...
    this.rows.forEach((row, rowIndex) => {
      ...
      this.columns.forEach((column, columnIndex) => {
        const isSelected = this.row === rowIndex && this.column === columnIndex;
        const value =
          (this.values[rowIndex] && this.values[rowIndex][columnIndex]) || "";
      ...
      const chalkModifier =
        this.row === rowIndex ? chalk.reset.bold.cyan : chalk.reset;
